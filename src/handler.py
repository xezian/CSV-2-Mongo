...
def handle_csv_upload(event, context):
    ...
    for entry in entries:
        ...
        if valid_manager_email:
            valid_manager = db.user.find_one({"normalized_email": valid_manager_email})
            if valid_manager:
                update_user["manager_id"] = valid_manager["_id"]
        if valid_hire_date:
        ...
        if valid_manager:
            ...
            manager_chain = db.chain_of_command.find_one(
                {"user_id": valid_manager["_id"]}
            )
            managers = manager_chain["chain_of_command"]
            managers.append(valid_manager["_id"])
